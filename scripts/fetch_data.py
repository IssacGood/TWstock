...
"""
台股全量資料抓取腳本
K棒只存 OHLCV（大幅減小檔案），技術指標由前端 JS 即時計算
...
def safe(v):
    ...
    try:
        f = float(v)
        return None if f != f else round(f, 2)  # 只保留2位小數，省空間
    except: return None
...
def fetch_stock(symbol, zh_name=""):
    try:
        ...
        # 只計算最後一天的指標（用來建篩選 flags）
        ma5  = sma(c, 5);  ma20 = sma(c, 20); ma60 = sma(c, 60)
        r14  = rsi(c, 14)
        ml   = macd_line(c); ms = ema(ml, 9)
        mid  = sma(c, 20);  std = c.rolling(20).std()
        bbu  = mid + 2*std; bbl = mid - 2*std
        vm20 = df["Volume"].rolling(20).mean()

        df["_ma5"]  = ma5;  df["_ma20"] = ma20; df["_ma60"] = ma60
        df["_rsi"]  = r14;  df["_macd"] = ml;   df["_macds"] = ms
        df["_bbu"]  = bbu;  df["_bbl"]  = bbl;  df["_vm20"] = vm20
        ...
        last_indicators = {
            ...
            "macd": safe(last._macd), "bbu":  safe(last._bbu),
            "bbl":  safe(last._bbl),
        }
        ...
        return {
            ...
            "ind":     last_indicators,
            "flags":   flags,
            "candles": candles,   # [[date,o,h,l,c,v], ...]
            # batch 欄位由主程式寫入
        }
    except: return None
...
def main():
    ...
    for i, (symbol, zh_name) in enumerate(batch):
        ...
        if data:
            data['batch'] = args.batch   # 讓前端知道 K棒在哪個檔案
            results.append(data)
    ...
    print(f"\nBatch {args.batch} 完成：{len(results)} 檔 / 失敗 {len(failures)} 檔")
    print(f"檔案大小：{size_mb:.1f} MB → {outpath}")
